...
/**
 * MarketplaceWebServiceWebstore_Client is an implementation of MarketplaceWebServiceWebstore
 *
 */
class MarketplaceWebServiceWebstore_Client implements MarketplaceWebServiceWebstore_Interface
{

    const SERVICE_VERSION = '2014-09-01';
    ...
    private  $_config = array ('ServiceURL' => null,
                               'UserAgent' => 'MarketplaceWebServiceWebstore PHP5 Library',
                               'SignatureVersion' => 2,
    ...
    private function _convertGetSubscriptionDetails($request) {
        ...
        if ($request->isSetSubscriptionState()) {
            $parameters['SubscriptionState'] =  $request->getSubscriptionState();
        }
    ...
    public function listSubscriptionsCount($request)
    {
        ...
        $parameters = $request->toQueryParameterArray();
        $parameters['Action'] = 'ListSubscriptionsCount';
        $httpResponse = $this->_invoke($parameters);
    ...
    /**
     * Convert ListSubscriptionsCountRequest to name value pairs
     */
    private function _convertListSubscriptionsCount($request) {
        ...
        if ($request->isSetSellerSKUList()) {
            $parameters['SellerSKUList'] =  $request->getSellerSKUList();
        }
    ...
    public function listSubscriptionsCountByNextToken($request)
    {
        ...
        $parameters = $request->toQueryParameterArray();
        $parameters['Action'] = 'ListSubscriptionsCountByNextToken';
        $httpResponse = $this->_invoke($parameters);
    ...
    /**
     * Convert ListSubscriptionsCountByNextTokenRequest to name value pairs
     */
    private function _convertListSubscriptionsCountByNextToken($request) {
   ...
   private function collapseWhitespace($s) {
    ...
    private function _invoke(array $parameters)
    {
        try {
            if (empty($this->_config['ServiceURL'])) {
                ...
                throw new MarketplaceWebServiceWebstore_Exception(
        ...
        } catch (MarketplaceWebServiceWebstore_Exception $se) {
            throw $se;
        } catch (Exception $t) {
            require_once (dirname(__FILE__) . '/Exception.php');
            throw new MarketplaceWebServiceWebstore_Exception(array('Exception' => $t, 'Message' => $t->getMessage()));
        }
    }
    ...
    private function _reportAnyErrors($responseBody, $status, $responseHeaderMetadata, Exception $e =  null)
    {
        ...
        require_once (dirname(__FILE__) . '/Exception.php');
        return new MarketplaceWebServiceWebstore_Exception($exProps);
    }
    ...
    private function _httpPost(array $parameters)
    {
        ...
        if($response === false) {
            ...
            curl_close($ch);
            throw new MarketplaceWebServiceWebstore_Exception($exProps);
        }
    ...
    */
    private function _extractResponseHeaderMetadata($rawHeaders){
        ...
        return new MarketplaceWebServiceWebstore_Model_ResponseHeaderMetadata(
